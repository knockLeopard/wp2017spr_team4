...
static DEFINE_SPINLOCK(setweight_lock);
...
asmlinkage long sys_sched_setweight(pid_t pid, int weight)
{
	...
	spin_lock(&setweight_lock);

        rq = task_rq(task);
        wrr_rq = &rq->wrr;
        wrr_rq->weight_sum -=prev_weight;
        wrr_rq->weight_sum += weight;
        task->wrr.time_slice = (weight * HZ) / 100;

	spin_unlock(&setweight_lock);
...
void sched_fork(struct task_struct *p)
{
	...
	if(!rt_prio(p->prio) && p->policy == 6) // MODIFIED
...
extern void wrr_load_balance(int);
...
recheck:
	...
	if (policy < 0) {
		...
		if (policy != SCHED_FIFO && policy != SCHED_RR &&
				...
				policy != SCHED_IDLE && policy != SCHED_WRR) // MODIFIED.
			return -EINVAL;
	}
